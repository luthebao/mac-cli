...
@(private="file") g_all_scanners: [21]types.Scanner
...
all_scanners :: proc() -> []types.Scanner {
	if !g_inited {
		...
		g_all_scanners[13] = {category = types.category_of(.Language_Files),      scan = language_files_scan,      clean = language_files_clean}
		g_all_scanners[14] = {category = types.category_of(.Large_Files),         scan = large_files_scan,         clean = large_files_clean}
		g_all_scanners[15] = {category = types.category_of(.Duplicates),          scan = duplicates_scan,          clean = duplicates_clean}
		g_all_scanners[16] = {category = types.category_of(.App_Cache),           scan = app_cache_scan,           clean = app_cache_clean}
		g_all_scanners[17] = {category = types.category_of(.System_Cache_Root),   scan = system_cache_root_scan,   clean = system_cache_root_clean}
		g_all_scanners[18] = {category = types.category_of(.Homebrew_Cleanup),    scan = homebrew_cleanup_scan,    clean = homebrew_cleanup_clean}
		g_all_scanners[19] = {category = types.category_of(.Homebrew_Autoremove), scan = homebrew_autoremove_scan, clean = homebrew_autoremove_clean}
		g_all_scanners[20] = {category = types.category_of(.Orphan_Symlinks),     scan = orphan_symlinks_scan,     clean = orphan_symlinks_clean}
		g_inited = true
	}
