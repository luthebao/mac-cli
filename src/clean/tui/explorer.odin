...
// — true means "selected for cleaning".
explore :: proc(category_name: string, parent_items: []types.CleanableItem) -> (selected: []bool, cancelled: bool) {
	rows := make([]CheckboxItem, len(parent_items), context.temp_allocator)
	for item, i in parent_items {
		rows[i] = CheckboxItem{
			label = abbreviate_path(item.path),
